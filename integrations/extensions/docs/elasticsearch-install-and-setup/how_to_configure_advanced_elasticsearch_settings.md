...
By default, keyword search is used for your Search integration, but you can configure the query body in the `Advanced Elasticsearch Settings` to enable advanced search such as semantic search with ELSER, KNN dense vector search, using a nested query to search nested documents, and hybrid search. Here are some query body examples:
